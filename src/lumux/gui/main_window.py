...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _should_minimize_to_tray(self) -> bool:
        return getattr(self.settings, "ui", None) is not None and getattr(
            self.settings.ui, "minimize_to_tray_on_sync", False
        )
    ...
    def _build_ui(self):
        ...
        self.status_subtitle = Gtk.Label(
            label="Connect to your Hue bridge to get started"
        )
        ...
        self.zone_preview = ZonePreviewWidget(
            rows=self.settings.zones.rows, cols=self.settings.zones.cols
        )
        ...
        default_rgb = xy_to_rgb(default_xy[0], default_xy[1])
        ...
        self.brightness_value_label = Gtk.Label(
            label=str(self.settings.reading_mode.brightness)
        )
    ...
    def _on_brightness_changed(self, scale):
        ...
        self._brightness_timeout_id = GLib.timeout_add(
            150, self._on_brightness_change_done
        )
    ...
    def _apply_reading_settings(self):
        ...
        if not self.mode_manager.is_reading_active():
            ...
            self.mode_manager.get_reading_controller().update_color(
                xy=xy, brightness=brightness
            )
    ...
    def _on_bridge_status_received(self, status):
        ...
        if self.bridge_connected:
            if status.entertainment_zone_name:
                channels = (
                    f"{status.entertainment_channel_count} channel(s)"
                    if status.entertainment_channel_count
                    else ""
                )
                self.status_label.set_text("Connected")
                self.status_subtitle.set_text(
                    f"Zone: {status.entertainment_zone_name} • {channels}"
                )
            else:
                self.status_label.set_text("Connected")
            ...
            if not status.configured:
                ...
                self.status_subtitle.set_text(
                    f"Cannot reach bridge at {status.bridge_ip}"
                )
        ...
        if hasattr(self, "open_bridge_settings_btn"):
            show_btn = (not self.bridge_connected) or (
                not getattr(status, "configured", True)
            )
        ...
        if last_status:
            ...
            if status_type == "status":
                if message == "syncing":
                    ...
                    if zone_colors and getattr(
                        self.settings.zones, "show_preview", True
                    ):
                        try:
    ...
    def _on_stop_clicked(self, button):
        ...
        auto_activate = getattr(self.settings, "reading_mode", None) and getattr(
            self.settings.reading_mode, "auto_activate", False
        )
    ...
    def _on_settings_closed(self, dialog=None):
        ...
        self._on_mode_changed(self.mode_manager.current_mode)
